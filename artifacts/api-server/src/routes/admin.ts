...
router.post("/admin/fetch-app-json", requireAdmin, async (req, res): Promise<void> => {
  ...
  try {
    ...
    if (!match) {
      res.status(400).json({ error: "Must be a valid GitHub repository URL" });
      return;
    }
    ...
    if (!appJson) {
      res.status(400).json({ error: "Could not find app.json in this repository (tried main and master branches)" });
      return;
    }
...
router.post("/admin/deployments/:id/suspend", requireAdmin, async (req, res): Promise<void> => {
  ...
  res.json({
    ...
    herokuAppId: updated.herokuAppId ?? null,
